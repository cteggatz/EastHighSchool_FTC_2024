...
module gear (
	...
	hole_diameter   = 6,    //diameter of the hole in the center, in mm
	twist           = 0,    //teeth rotate this many degrees from bottom of gear to top.  360 makes the gear a screw with each thread going around once
	teeth_to_hide   = 0,    //number of teeth to delete to make this only a fraction of a circle
	pressure_angle  = 14,   //28 Controls how straight or bulged the tooth sides are. In degrees.
...
mm_per_tooth = 4; //all meshing gears need the same mm_per_tooth (and the same pressure_angle)
thickness    = 24; //Height of the overall gear
hole         = 6.5; //Hole down the middle of the gear
baseheight=12; //height of the bottom of the gear
...
d14=pitch_radius(mm_per_tooth,n1) + pitch_radius(mm_per_tooth,n4);
 ...
 difference(){
     union(){
     union() {
 //Make base cylinder and cut away middle hole
   
        difference(){
    translate([0,0,-6]) cylinder(h=baseheight, r=10, center=true, $fn=100);
        cylinder(h=thickness, r=hole/2, center=true, $fn=100);
        }
...
//Make Dshaft cube
translate([-2.5,2.5,-thickness/2]) cube([5,2.5,thickness]);
   
}
//cut pinion hole
translate([0,0,-6]) rotate ([90, 90, 00]) cylinder(h=100, r=3.3/2, center=true, $fn=100);
}
