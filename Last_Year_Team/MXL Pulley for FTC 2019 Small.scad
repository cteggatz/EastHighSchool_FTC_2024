...
motor_shaft = 7.9375;	// NEMA17 motor shaft exact diameter = 5
...
if ( profile == 1 ) { 
    ...
    pulley ( "MXL" , MXL_pulley_dia , 0.508 , 1.321 );
        //David says this is the retainer flange for the bottom
    cylinder(d1=25,d2=23,h=1.5);
    }
    //David says these are the holes for the screws
    translate([.625/2*25.4,0,-2])
    cylinder(d=0.145*25.4,h=100,$fn=64);
    translate([-.625/2*25.4,0,-2])
    cylinder(d=0.145*25.4,h=100,$fn=64); 
    translate([0,.625/2*25,-2])
    cylinder(d=0.145*25.4,h=100,$fn=64); 
    translate([0,-.625/2*25,-2])
    cylinder(d=0.145*25.4,h=100,$fn=64); 
    //David says this is the cut for the throughhole
    translate([0,0,-2])
    cylinder(d=5/16*25.3,h=100,$fn=64); 
    }
